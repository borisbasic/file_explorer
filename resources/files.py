...
class AddFileByUser(MethodView):
    ...
    def post(self, file_data):
        ...
        file.save(
            os.path.join(
                docs_dir + "/" + user.username_uuid,
                secure_filename(str(name_uuid + file.filename)),
            )
        )
        new_file = FilesModel(
            name=name_uuid + file.filename,
            name_uuid=name_uuid,
...
class UserFile(MethodView):
    ...
    def get(self, file_name_uuid):
        username_uuid = get_jwt_identity()
        file = FilesModel.query.filter(
            and_(
                FilesModel.username_uuid == username_uuid,
                FilesModel.name_uuid == file_name_uuid,
            )
        ).first()
        if not file:
    ...
    def put(self, file_data, file_name_uuid):
        ...
        new_file.save(
            os.path.join(
                docs_dir + "/" + username_uuid,
                secure_filename(str(name_uuid + new_file.filename)),
            )
        )
